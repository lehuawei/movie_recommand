...
Page({
    ...
    onLoad: function(options) {
		...
		douban.fetchFilmDetail.call(that, config.apiList.filmDetail, id, function(data){
			/// 判断是否收藏
			if(app.globalData.collectMovie.indexOf(data.id)>=0){
				that.setData({
					isFilmFavorite: true
				})
			}
	...
	favoriteFilm: function() {
		var that = this
		// 判断原来是否收藏，是则删除，否则添加
		let postData = "movieId="+that.data.filmDetail.id;
		if(that.data.isFilmFavorite){
			postData += "&api=disCollectMovie";

		}else{
			postData += "&api=collectMovie";
		}
		douban.HttpGet(postData).then((res)=>{
			if(that.data.isFilmFavorite){
				for (var i = 0; i < app.globalData.collectMovie.length; i++) {
					if (app.globalData.collectMovie[i] == that.data.filmDetail.id) {
						app.globalData.collectMovie.splice(i,1)
				...
				app.globalData.collectMovie.push(that.data.filmDetail.id);
				that.setData({
					isFilmFavorite: true
				})
			}
